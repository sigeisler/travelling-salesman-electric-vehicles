...
os.execute("apk add lua-sql-postgres")
lua_sql = require "luasql.postgres"
sql_env = assert( lua_sql.postgres() )
sql_con = assert( sql_env:connect("postgres", "postgres", "mysecretpassword", "elevation_postgis", 5432) )
...
function query_elevation(lat, lon)
  -- queries elevation data from postgis
  local sql_query = " " ..
    "SELECT AVG(ST_Value(rast, loc.geom)) AS elevation " .. 
    "FROM public.elevation_raster, " ..
    " (SELECT ST_GeomFromText('Point (" ..
    lon .. " " .. lat .. 
    ")', 4326) AS geom) AS loc " ..
    "WHERE ST_Intersects(rast, loc.geom)"

  local cursor = assert( sql_con:execute(sql_query) )
  local row = cursor:fetch( {}, "a" )
  if row then
    if row.elevation == nil then
      return -1
    end
    local elevation = tonumber(row.elevation)
    cursor:close()
    return elevation
  else
    cursor:close()
...
function elevation(lat, lon)
  -- warpper for exception handling
  local status, value = pcall(query_elevation, lat, lon)
  if status then
    return value
  else 
    print("Elevation query failed: " .. value)
    return -1
  end
end

function process_segment (profile, segment)
  -- penalizing steep slopes for electric vehicles
  local sourceElevation = elevation(segment.source.lat, segment.source.lon)
  local targetElevation = elevation(segment.target.lat, segment.target.lon)
